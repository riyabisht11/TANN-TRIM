    ...
    <div id="payment-method" className="shadow-[0px_0px_26px_5px_rgba(0,_0,_0,_0.1)]  rounded-xl inline-block  overflow-hidden ">
      <main className="px-6 md:px-12 py-7 space-y-12">
        ...
        {array.map((item, idx) => (
          <div 
          className={`
            px-7 py-5 flex justify-between cursor-pointer 
            hover:bg-black/20 hover:text-white w-full lg:w-[648px]
            ${idx === 0 ? "hover:rounded-t-lg" : ""}
            ${idx === array.length - 1 ? "hover:rounded-b-lg" : "border-b border-[#E7E7E7]"}
          `}
        >
